...
TEST(MapBuilder, Reset) {
	MapBuilder *builder = new MapBuilder();
	builder->addBox(1, 2)->addTarget(1, 3)->addBlock(2, 2)->addBlock(3, 2)->setPlayer(
			1, 1);
...
TEST(MapBuilder, Build) {
	MapBuilder *builder = new MapBuilder();
	builder->addBox(2, 1)->addTarget(3, 1)->addBlocks(1, 0, 3, 0)->addBlocks(1,
			2, 3, 2)->addBlock(0, 1)->addBlock(4, 1)->setPlayer(1, 1);
