...
LevelSelector::LevelSelector(const std::string &path, WINDOW *window,
		...
		_path(path), _relativePath(), _callback(callback), _index(0), _window(
				window), _rectangle(0, 0,
				SokobanLevelSelector::WINDOW_WIDTH + 30,
				SokobanLevelSelector::DISPLAYED_ROWS), _maxFileName(
				SokobanLevelSelector::WINDOW_WIDTH), _enabled(false) {
...
LevelSelector::~LevelSelector() {
	if (_display) {
		const IMap *map = _display->getMap();
		if (map)
			delete map;
	...
	case SokobanLevelSelector::Input::Select:
      ...
      if (_relativePath.find('\\') == std::string::npos) {
			...
			if (_relativePath.find('/') == std::string::npos) {
				...
				_relativePath = _relativePath.substr(0,
						_relativePath.find('/'));
			}
...
void LevelSelector::update() {
	...
	for (uint32_t index = 0; index < displayedItems; index++) {
		...
		if (line) {
			waddstr(_window,
					(line->length() <= _maxFileName) ?
							line->c_str() :
							line->substr(0, _maxFileName).c_str());
		}
	}
	...
	if (((parent && _index != 0) || !parent)
		...
		try {
			const Map *loadedMap = _expandedParser.readFile(
					mapEntry->getPath());
