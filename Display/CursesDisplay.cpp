...
CursesDisplay::CursesDisplay(WINDOW *window) :
		_window(window), _map(nullptr), _staticMap(nullptr), _state(nullptr), _enabled(
				false), _rectangle(0, 0, 30, 20) {
	_valid = true;
}
...
void CursesDisplay::setMap(const IMap *map) {
	if ((map != _map) && (_map))
		delete _map;
...
void CursesDisplay::updateState(const IState *state) {
	_state = state;

	update();
}

const IMap* CursesDisplay::getMap() const {
	return _map;
}
const IState* CursesDisplay::getState() const {
	return _state;
}
...
char CursesDisplay::getCharForState(StaticType type, const IState *state,
